...
describe "operation" (fun _ => {
  ...
  test "dropLastOne returns the list except the last one" (fun _ => {
    let actual = Operation.dropLastOne [ 1, 2, 3, 4 ];
    let expected = [ 1, 2, 3 ];
  ...
  test "last returns last item in the list" (fun _ => {
    let { left, right, symbol, total }: Operation.model = Operation.last [
    ...
    let actual = ( left, right, symbol, total );
    let expected = ( "4", "", Action.Pending, 0 );

    expect actual |> toEqual expected;
  });

  test "findLast returns last item matching the predicate" (fun _ => {
    let lst = [
    ...
    let { left, right, symbol, total }: Operation.model =
      Operation.findLast lst (fun { symbol } => Pervasives.(symbol == Action.Add));
    let actual = ( left, right, symbol, total );
